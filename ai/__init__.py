...
分析必须详尽，每个结论都要给出具体的数据依据和逻辑推理，禁止只下结论不解释原因：
1. 逐项分析每个技术指标（均线/MACD/KDJ/RSI/布林带/量价）的当前数值、状态、多空含义、是否触发信号及原因
2. 汇总触发的买入/卖出信号，每条信号说明具体依据
3. 操作建议（买入/卖出/观望）及综合理由
4. 入场价格区间及选择依据
5. 止损位（必须设置）及设置依据
6. 止盈目标（至少2个）及依据
7. 信号强度（强/中/弱）及判定理由
...
只输出JSON，不要其他内容。"""
    ...
    def _build_prompt(self, data: Dict, analysis_type: str) -> str:
        ...
        # 分析要求
        prompt += """【请按以下格式输出分析，每一项都必须给出具体数据依据和逻辑推理】

1. **当前状态**：1句话概括 + 判断依据
2. **技术指标逐一分析**（每个指标写明：当前数值 -> 状态 -> 多空含义 -> 是否触发信号及原因）：
   - 均线系统：MA5/MA10/MA20 数值、排列方式、价格与均线关系
   - MACD：DIF/DEA/柱状线数值、金叉/死叉、柱状线变化趋势
   - KDJ：K/D/J 数值、超买超卖区、交叉情况
   - RSI：数值、超买超卖、趋势
   - 布林带：价格在带中位置、带宽变化
   - 量价关系：成交量变化、量价配合程度
3. **信号汇总**：列出触发的买入/卖出信号，每条说明具体依据
4. **操作建议**：买入 / 卖出 / 观望，并给出综合理由
5. **入场价格**：具体区间 + 选择依据
6. **止损位**：具体价格 + 设置依据（关键支撑/ATR/均线）
7. **止盈目标**：至少2个目标位 + 依据（前高/布林上轨/压力位）
8. **信号强度**：强 / 中 / 弱 + 判定理由（信号数量、指标共振程度）
9. **风险收益比**：具体计算
10. **风险提示**：具体风险点 + 信号有效期（1-3天）

要求：
- 每个结论必须引用具体数值或交叉关系作为依据，解释"为什么"
- 禁止只给结论不分析原因
- 止损止盈必须给出具体价格和设置逻辑
- 如果是观望，说明等待什么条件触发"""
